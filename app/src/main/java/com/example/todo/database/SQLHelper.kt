...
class SQLHelper(context:Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onCreate(database: SQLiteDatabase?) {
        val crateTable = ("CREATE TABLE " + TABLE + "(" + ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + TITLE + " TEXT, " + DESC + " TEXT, " + DATE + " INTEGER, " + PRIO + " INTEGER " + ")")
    ...
    fun displayEvents(): ArrayList<EventModel>{
        ...
        if(cursor.moveToFirst()) {
            do {
                ...
                val event = EventModel(event_id = id, event_title = title, event_desc = desc, event_date = date, event_prio = prio)
                events.add(event)
